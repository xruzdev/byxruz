 ...
 */
export function horizontalLoopDraggable(
  items: HTMLElement[],
  ...
  gsap.context(() => {
    ...
    const getTotalWidth = () =>
        ...
        items[length - 1].offsetWidth *
          Number(gsap.getProperty(items[length - 1], "scaleX")) +
        (parseFloat((config.paddingRight as string) || "0") || 0),
      populateWidths = () => {
        let b1 = container.getBoundingClientRect(),
          b2;
        items.forEach((el, i) => {
          widths[i] = parseFloat(
            gsap.getProperty(el, "width", "px") as unknown as string
          );
          xPercents[i] = snap(
    ...
    function toIndex(index: number, vars: gsap.TweenVars) {
      ...
      //
      // s go in the shortest direction
      const newIndex = gsap.utils.wrap(0, length, index);
      let time = times[newIndex];
