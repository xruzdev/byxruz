...
export const Menu = ({ desktopVisible = false, desktopMenuRef }: MenuProps) => {
  const localContainerRef = React.useRef<HTMLDivElement>(null);
  const containerRef = desktopMenuRef ?? localContainerRef;
  ...
  const { setCursorSize, setIsLenisStopped } = useAppStore();
  ...
  useEffect(() => {
    ...
    if (openMenu) {
      lenis.stop();
      setIsLenisStopped(true);
    } else {
      lenis.start();
      setIsLenisStopped(false);
    }

    return () => {
      setIsLenisStopped(false);
    };
  }, [openMenu, lenis, setIsLenisStopped]);
  ...
  const closeMenu = async () => {
    ...
    await tlMenuRef.current.reverse();
    setOpenMenu(false);
  ...
  useGSAP(
    () => {
      ...
      const linkItems = q(".link-item") as HTMLElement[];

      if (linkItems.length) {
        gsap.set(linkItems, { willChange: "transform" });
      }
      ...
      return () => {
        mm.revert();
        if (linkItems.length) {
          gsap.set(linkItems, { clearProps: "willChange" });
        }
      };
    },
    {
  ...
  return (
    <>
      <aside
        ...
        id="menu"
        className={
          "size-15 fixed top-6 md:top-10 z-555 lg:top-6 xl:top-10 right-4 md:right-10 lg:right-6 xl:right-10 flex flex-col justify-start items-end overflow-hidden rounded-xl md:rounded-2xl bg-transparent border border-main transition-opacity duration-300 ease-out " +
          (openMenu || desktopVisible
      ...
      >
        ...
        >
          ...
          <div
            ...
            className="btn-layer size-20 bg-main absolute top-1/2 left-1/2 -translate-1/2 -translate-x-1/2 z-0"
          ></div>
        </button>

        <div className="top-15 md:top-20 left-0  wrapper w-full h-[calc(100dvh-6rem)] md:h-[calc(100vh-8rem)]  px-6   md:px-10   absolute text-xl flex   flex-col justify-evenly items-center text-foreground      rounded-2xl md:rounded-3xl overflow-hidden">
          ...
          <ul className="flex items-center justify-evenly w-full  mb-10">
            {[
      ...
      <div
        ...
        className="fixed top-0 left-0 w-screen h-screen z-550 backdrop-blur-xs opacity-0 grayscale  ease-out bg-background/10"
      ></div>
    </>
  );
};
