...
import { sendContactEmailAction } from "@/lib/send-contact-email-action";
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setStatus(res.message || "Error al enviar el mensaje");
      }
    } catch {
  ...
  return (
    ...
    >
      <h1 className="text-6xl uppercase md:text-8xl 2xl:text-9xl font-bold translate-x-[-120%]">
        Hablemos<span className="text-main">.</span>
      </h1>
      <p className="text-lg md:text-xl mb-10 translate-x-full ">
      ...
      <div className="flex flex-col lg:flex-row w-full mb-20 items-start lg:justify-between">
        ...
        >
          <div className="w-full flex items-start   justify-start border-t mt-6 py-10 translate-x-[-150%] ">
            <span className="text-main">01</span>
          ...
          <div className="w-full flex items-start justify-start border-t py-10 translate-x-[-150%] ">
            <span className="text-main">02</span>
            <label className="flex flex-col w-full uppercase font-bold ml-4">
              ¿Cuál es tu email?
              <input
                type="email"
          ...
          <div className="w-full flex items-start justify-start border-y py-10 translate-x-[-150%] ">
            <span className="text-main">03</span>
            <label className="flex flex-col w-full uppercase font-bold ml-4">
              ¿Cuál es tu mensaje?
              <textarea
          ...
          <button
            ...
            className="mt-10 cursor-pointer px-6 py-3 bg-main text-background font-bold  flex items-center justify-center   uppercase rounded-lg lg:hover:bg-main/80    translate-x-[-200%]"
          >
            {status}{" "}
            {status === "enviando" && (
              <svg
        ...
        <div className="w-full lg:w-1/3 xl:w-1/4 flex flex-col items-start justify-center h-auto mt-10 lg:mt-0 lg:ml-10 info-container translate-x-[150%]">
          ...
          <h2 className="text-2xl font-bold mt-10 ">
            Zona Horaria <span className="text-main">GMT-3 </span>
          </h2>
